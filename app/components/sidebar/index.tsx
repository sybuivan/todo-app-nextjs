...
const Sidebar = () => {
  ...
  const { payloadFilters } = useAppSelector((state) => state.taskSlice);
  ...
  const handleOnSubmit = ({ name }: { name: string }) => {
    dispatch(createTaskType(name)).then((data: any) => {
      reset({ name: '' });
    });
  };
          ...
          <>
            {roles.includes(ROLE_USER) && (
              <div>
                ...
                <h2 className="font-bold py-2">TYPES</h2>
                <ul className="space-y-2 font-medium mb-9 border-b-2">
                  {taskTypeList.map((item) => (
                    <li
                      onClick={() => {
                        ...
                        handleOnChangeType(item.typeId);
                      }}
                      key={item.typeId}
                      className={`flex hover:cursor-pointer items-center p-2 text-gray-900 rounded-lg ${
                        payloadFilters.typeId == item.typeId && 'bg-gray-100'
                      }
